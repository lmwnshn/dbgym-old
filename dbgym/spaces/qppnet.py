...
class QPPNetFeatures(spaces.Sequence):
    ...
    def __init__(
        ...
        self._parent_relationships = ["invalid", "Inner", "Outer", "Subquery", "Member", "children", "child"]
        self._sort_methods = ["invalid", "still in progress", "top-N heapsort", "quicksort", "external sort", "external merge"]
        ...
        space_dict = {
            ...
            "Children Indexes": spaces.Sequence(spaces.Box(low=0, high=np.inf, dtype=np.int64, seed=seed), seed=seed),
            ...
            "Node Position": spaces.Sequence(spaces.Box(low=0, high=np.inf, dtype=np.int32, seed=seed), seed=seed),
            ...
            "Parent Relationship": spaces.MultiBinary(len(self._parent_relationships), seed=seed),
            ...
            "Attribute Mins": spaces.Box(low=-np.inf, high=np.inf, shape=(self._max_num_attributes,), seed=seed),
            "Attribute Medians": spaces.Box(low=-np.inf, high=np.inf, shape=(self._max_num_attributes,), seed=seed),
            "Attribute Maxs": spaces.Box(low=-np.inf, high=np.inf, shape=(self._max_num_attributes,), seed=seed),
            ...
            #  The original paper has no code, our reimplementation didn't have it either.
            # "Operation": ,
            "Actual Total Time": spaces.Box(low=0, high=np.inf, dtype=np.float32, seed=seed),
        }
    ...
    def generate(self, result_dict, query_num, observation_idx) -> list:
        ...
        node_position = [0]
        return self._generate(plan_dict, query_num, query_hash, node_position, observation_idx)
    ...
    def _generate(self, plan_dict, query_num, query_hash, node_position, observation_idx) -> list:
                ...
                if not rel_name.startswith("pg_"):
                    ...
                    for idx, attr_name in enumerate(attributes):
                        if attr_name in target:
                            ...
                            if type(self._attribute_stats[rel_name][attr_name]["min"]) == str:
                                continue
                            output_min_vec[idx] = self._attribute_stats[rel_name][attr_name]["min"]
                            output_med_vec[idx] = self._attribute_stats[rel_name][attr_name]["median"]
                            output_max_vec[idx] = self._attribute_stats[rel_name][attr_name]["max"]
        ...
        if "Plans" in plan_dict:
            for i, plan in enumerate(plan_dict["Plans"]):
                ...
                child_observations = self._generate(plan, query_num, query_hash, new_position, observation_idx)
                observations.extend(child_observations)
                observation_idx += len(child_observations)
        output_children_indexes = [_singleton(idx, dtype=np.int64) for idx in children_indexes]
        ...
        ordered_dict_items = [
            ...
            ("Hash Buckets", _singleton(plan_dict["Hash Buckets"] if "Hash Buckets" in plan_dict else 0)),
            ...
            ("Parent Relationship", _one_hot(self._parent_relationships, "Parent Relationship")),
