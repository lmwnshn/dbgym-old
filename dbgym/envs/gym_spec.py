...
class GymSpec:
    ...
    def _snapshot_db(self):
        ...
        for schema_name in self._inspector.get_schema_names():
            ...
            for (table_name, fkcs) in self._inspector.get_sorted_table_and_fkc_names(schema_name):
                if table_name is None:
                    # The last iteration consists of FKs that would require a separate CREATE statement.
                    continue
                table_attr: TableAttr = {}
                table_attr["columns"]: list[Column] = self._inspector.get_columns(table_name, schema_name)
                table_attr["indexes"]: list[Index] = self._inspector.get_indexes(table_name)
