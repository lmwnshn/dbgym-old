...
class QPPNet:
    def __init__(
        ...
        # Compute dimensions.
        self._dim_dict = self._compute_dim_dict(pd.concat([self._train_df, self._test_df]))
    ...
    def split(
        ...
        if min_train_size is not None:
            n_train = min_train_size if isinstance(min_train_size, int) else min_train_size * n_all
        if min_test_size is not None:
            n_test = min_test_size if isinstance(min_test_size, int) else min_test_size * n_all
        if n_train is None:
        ...
        assert n_train is not None and n_test is not None, "Need to specify either train size or test size."
        ...
        try:
            with torch.autograd.set_detect_anomaly(False):
                ...
                for epoch in tqdm(
                    ...
                    if early_stop:
                        print(f"Early stopping: epoch {epoch}, min val loss {self._min_validation_loss}.")
                        break
                    ...
                    for _, gdf in equivalence_classes:
                        ...
                        for node_type, obs_idx, children_idxs, features, actual_time in zip(
                            ...
                            for child_idx in children_idxs:
                                assert child_idx in output_cache, f"While computing {obs_idx}, {child_idx} not cached?"
                    ...
                    should_save = (epoch_save_interval is not None) and (epoch % epoch_save_interval == 0)
                    ...
                    if validation_df is not None:
                        ...
                        if validation_loss < threshold:
                            ...
                            patience -= 1
                            if patience == 0 and (
                                self._patience_min_epochs is None or epoch >= self._patience_min_epochs
                            ):
                                early_stop = True
    ...
    def evaluate(self, df: Optional[pd.DataFrame] = None, leave_tqdm: bool = True) -> pd.DataFrame:
        """
    ...
    def save_weights(self, epoch: int, metrics_str: str):
        for neural_unit_id, neural_unit in self._neural_units.items():
            filename = (
                f"{neural_unit_id.node_type}-{neural_unit_id.num_children}" f"_epoch_{epoch}_m_{metrics_str}_end.pth"
            )
    ...
    def calculate_root_mean_square_error(evaluate_df):
        return ((evaluate_df["Actual Latency (scaled)"] - evaluate_df["Estimated Latency (scaled)"]) ** 2).mean() ** 0.5
    ...
    def calculate_relative_error(evaluate_df):
        return (
            ...
            / len(evaluate_df)
            * (
                ((evaluate_df["Actual Latency (scaled)"] - evaluate_df["Estimated Latency (scaled)"]).abs())
    ...
    def calculate_mean_absolute_error(evaluate_df):
        return (
            1
            / len(evaluate_df)
            * (((evaluate_df["Actual Latency (scaled)"] - evaluate_df["Estimated Latency (scaled)"]).abs())).sum()
        )
    ...
    def calculate_rq(evaluate_df, ignore_zeros=True):
        ...
        if ignore_zeros:
            ...
            evaluate_df = evaluate_df[
                (evaluate_df["Estimated Latency (scaled)"] != 0) & (evaluate_df["Actual Latency (scaled)"] != 0)
            ]
    ...
    def _compute_dim_dict(df: pd.DataFrame) -> dict[NeuralUnitId, int]:
        ...
        for node_type, num_children, feature_len in zip(
            ...
            if neural_unit_id in dim_dict:
                assert dim_dict[neural_unit_id] == input_length, f"Disagreement on input length?"
            else:
